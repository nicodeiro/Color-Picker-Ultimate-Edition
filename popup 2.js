document.addEventListener('DOMContentLoaded', () => {
    ...
    const DEFAULT_HISTORY = ['#2563EB', '#7C3AED', '#FF3B5F', '#FF7A1A', '#10B981', '#06B6D4', '#F9AB00', '#1F2937', '#CBD5E1', '#E8E5F4'];
    ...
    const primaryColorCard = document.getElementById('primary-color-card');
    ...
    const translations = {
        ...
        fr: {
            ...
            colorPreview: 'Couleur',
            uiPreview: 'Aperçu UI',
            previewButton: 'Bouton',
            previewLabel: 'Label',
            previewText: 'Texte en couleur',
            history: 'Historique',
    ...
    document.getElementById('bookmark-btn').addEventListener('click', () => {
        if (currentColor) {
            ...
            showToast(translations[settings.language].copied || 'Saved!');
        }
    });
    ...
    function setColor(hex) {
        ...
        colorPreview.style.setProperty('--selected-color', currentColor);
        colorPreview.classList.add('has-color');
        primaryColorCard.classList.remove('hidden');
        ...
        oklchValue.textContent = currentColor === DEFAULT_COLOR ? '0.62, 0.16, 250' : hexToOklch(hex);
        renderHistory();
    }
    ...
    function renderHistory() {
        ...
        colorHistory.slice(0, MAX_HISTORY).forEach(hex => {
            if (!isValidHex(hex)) return;
            const div = document.createElement('div');
            div.className = `history-color${currentColor && currentColor.toLowerCase() === hex.toLowerCase() ? ' active' : ''}`;
    ...
    function loadColorHistory() {
        storage.get(['colorHistory'], (result) => {
            colorHistory = Array.isArray(result.colorHistory) && result.colorHistory.length
                ? result.colorHistory.slice(0, MAX_HISTORY)
                : DEFAULT_HISTORY;
            renderHistory();
        });
    }
    ...
    function validateSettings(s) {
        ...
        // Validate Theme
        if (s.theme && !VALID_THEMES.includes(s.theme)) return false;
    ...
    function applyTranslations() {
        ...
        document.querySelectorAll('[data-i18n]').forEach(el => {
            ...
            if (el.classList.contains('promo-link')) {
                renderPromoLink(el, t(key));
                return;
            }
    ...
    function renderPromoLink(el, text) {
        el.textContent = '';
        const bitekIndex = text.indexOf('bitek.fr');
        if (bitekIndex === -1) {
            el.textContent = text;
            return;
        }

        el.append(document.createTextNode(text.slice(0, bitekIndex)));
        const strong = document.createElement('span');
        strong.className = 'promo-domain';
        strong.textContent = 'bitek.fr';
        el.append(strong);
        el.append(document.createTextNode(text.slice(bitekIndex + 'bitek.fr'.length)));
    }
});
